...
def get_config_path() -> Path:
    ...
    return Path.home() / ".mlpcopilot" / "config.json"
...
def load_config(config_path: Path | None = None) -> Config:
    ...
    if path.exists():
        try:
            ...
            if _targets_mlpcopilot_profile(data):
                raise
            logger.warning(f"Failed to load config from {path}: {e}")
            logger.warning("Using default configuration.")
...
def _apply_ssrf_whitelist(config: Config) -> None:
    ...
    from mlpcopilot.security.network import configure_ssrf_whitelist

    configure_ssrf_whitelist(config.tools.ssrf_whitelist)


def _targets_mlpcopilot_profile(data: dict[str, Any] | None) -> bool:
    if data is None:
        return False
    profile = data.get("runtimeProfile", data.get("runtime_profile", MLPCOPILOT_PROFILE))
...
def _migrate_config(data: dict) -> dict:
    ...
    if isinstance(exec_cfg, dict):
        exec_cfg.setdefault("allowCommands", [])
