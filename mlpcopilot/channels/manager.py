...
def _default_webui_dist() -> Path | None:
    ...
    try:
        import mlpcopilot.web as web_pkg  # type: ignore[import-not-found]
    except ImportError:
        return None
...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        runtime_profile = getattr(self.config, "runtime_profile", "default")
        for name, cls in discover_all().items():
            if not channel_allowed_for_profile(runtime_profile, name):
                logger.info(
                    "{} channel skipped by runtime profile {}",
                    name,
                    runtime_profile,
                )
                continue
            ...
            try:
                ...
                channel.runtime_config = self.config
