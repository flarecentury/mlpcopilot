...
from mlpcopilot.command.builtin import build_help_text
from mlpcopilot.config.paths import get_media_dir
from mlpcopilot.config.schema import Base
from mlpcopilot.security.network import validate_url_target
from mlpcopilot.utils.helpers import split_message
...
class TelegramChannel(BaseChannel):
    ...
    BOT_COMMANDS = [
        ...
        BotCommand("runs", "Show recent run manifests"),
        BotCommand("artifacts", "Show artifacts for a run"),
        BotCommand("approvals", "Show pending approvals"),
        BotCommand("approve", "Approve a pending decision"),
        BotCommand("reject", "Reject a pending decision"),
        BotCommand("changes", "Request changes for a pending decision"),
    ...
    def _normalize_telegram_command(content: str) -> str:
        """Map Telegram-safe command aliases back to canonical mlpcopilot commands."""
    ...
    async def start(self) -> None:
        ...
        self._app.add_handler(
            MessageHandler(
                filters.Regex(r"^/(new|stop|restart|status|model|history|dream|runs|artifacts|approvals|approve|reject|changes)(?:@\w+)?(?:\s+.*)?$"),
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm mlpcopilot.\n\n"
    ...
    async def _on_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not update.message:
            return
        await update.message.reply_text(build_help_text(self.runtime_config))
