...
class FeishuChannel(BaseChannel):
    ...
    async def _add_reaction(self, message_id: str, emoji_type: str = "THUMBSUP") -> str | None:
        ...
        return self._add_reaction_sync(message_id, emoji_type)
    ...
    async def _remove_reaction(self, message_id: str, reaction_id: str) -> None:
        ...
        self._remove_reaction_sync(message_id, reaction_id)
    ...
    async def send_delta(
        ...
        if meta.get("_stream_end"):
            ...
            if buf.card_id:
                buf.sequence += 1
                ok = self._stream_update_text_sync(buf.card_id, buf.text, buf.sequence)
                if ok:
                    buf.sequence += 1
        ...
        if buf.card_id is None:
            ...
            card_id = self._create_streaming_card_sync(
                rid_type,
                chat_id,
                reply_msg_id,
            ...
            if card_id:
                buf.card_id = card_id
                buf.sequence = 1
                self._stream_update_text_sync(card_id, buf.text, 1)
                buf.last_edit = now
        ...
        try:
            ...
            if msg.metadata.get("_tool_hint"):
                ...
                if _th_msg_id:
                    ...
                    self._send_message_sync(receive_id_type, msg.chat_id, "interactive", card)
                return
