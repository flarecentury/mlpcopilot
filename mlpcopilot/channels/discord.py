...
from mlpcopilot.utils.helpers import safe_filename, split_message
    ...
    class DiscordBotClient(discord.Client):
        ...
        def _register_app_commands(self) -> None:
            commands = (
                ...
                ("dream", "Manually trigger Dream consolidation", "/dream"),
            )
            ...
            @self.tree.command(name="model", description="Show or switch model")
            @app_commands.describe(model="Model name to switch to")
            async def model_command(interaction: discord.Interaction, model: str = "") -> None:
                command_text = f"/model {model}".strip()
            ...
            async def help_command(interaction: discord.Interaction) -> None:
                ...
                if not await self._interaction_channel_allowed(interaction, channel):
                    await self._reply_ephemeral(interaction, "This channel is not allowed for this bot.")
                    return
                await self._reply_ephemeral(interaction, build_help_text(self._channel.runtime_config))
        ...
        async def send_outbound(self, msg: OutboundMessage) -> None:
            """Send a mlpcopilot outbound message using Discord transport rules."""
...
class DiscordChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not DISCORD_AVAILABLE:
            logger.error("discord.py not installed. Run: pip install mlpcopilot[discord]")
            return
