---
name: my
description: Check and set the agent's own runtime state (model, iterations, context window, token usage, web config). Use when diagnosing why something doesn't work ("why can't you search the web?", "why did you stop?"), checking resource limits before complex tasks, adapting configuration for long or simple tasks, or holding temporary per-session state. Also use when the user asks what model you are running, how many tokens you've used, or what your settings are.
always: true
---
...
<rule>
**Recall within this runtime session.** Store temporary scratchpad values here, read them back later.
</rule>
...
| Current goal/plan/project pointer | `workstate` | Session |
