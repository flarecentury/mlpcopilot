...
Use `exec` to run `mlpcopilot --version`. Tell the user the current version.
...
```
question: "How did you install mlpcopilot?"
options: ["uv", "pipx", "pip", "source (git clone)"]
```

If the user selected `source (git clone)`, ask for the local checkout path:
`question: "Where is your mlpcopilot source checkout? Enter an absolute path or a path relative to this workspace:"`.
...
| Method | Command |
|--------|---------|
| uv | `uv tool install "mlpcopilot[EXTRAS]" --force` |
| pipx | `pipx install --force "mlpcopilot[EXTRAS]"` |
| pip | `python -m pip install --upgrade "mlpcopilot[EXTRAS]"` |
...
3. Use `exec` to verify: `mlpcopilot --version`
4. Tell the user the new version. Say: "Run `/restart` to restart mlpcopilot and apply the update. If `/restart` is unavailable in this channel, restart the mlpcopilot process manually."
```

## Step 5: Confirm

Tell the user: "Upgrade skill created. Say 'upgrade mlpcopilot' when you want to update."
