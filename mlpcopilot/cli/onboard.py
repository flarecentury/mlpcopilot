"""Interactive onboarding questionnaire for mlpcopilot."""
...
from mlpcopilot.cli.models import (
    format_token_count,
...
def _show_main_menu_header() -> None:
    ...
    console.print(Align.center(f"[bold cyan]MLP Copilot v{__version__}[/bold cyan]"))
    console.print()
...
def _try_auto_fill_context_window(model: BaseModel, new_model_name: str) -> None:
    ...
    Note:
        This function imports AgentDefaults from mlpcopilot.config.schema to get
...
def _get_provider_info() -> dict[str, tuple[str, bool, bool, str]]:
    ...
    from mlpcopilot.providers.registry import PROVIDERS
...
def _get_channel_info() -> dict[str, tuple[str, type[BaseModel]]]:
    ...
    for name, channel_cls in discover_all().items():
        try:
            mod = importlib.import_module(f"mlpcopilot.channels.{name}")
