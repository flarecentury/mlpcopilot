...
MLP/DP-GEN memory hygiene:
- Do not promote live DP-GEN status into durable memory: current iteration, current stage, queue counts, candidate/accurate/failed counts, transient dispatcher errors, or temporary traceback snippets.
- Preserve only stable project facts, user preferences, durable paths, approved decisions, acceptance criteria references, and confirmed site conventions.
- If a DP-GEN fact is useful but likely dynamic, output [SKIP] unless the user explicitly asked to preserve it as historical context.
