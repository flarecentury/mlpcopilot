"""MCP client: connects to MCP servers and wraps their tools as native mlpcopilot tools."""
...
def _open_stdio_errlog(stack: AsyncExitStack, server_name: str):
    """Open a non-terminal stderr sink for an MCP stdio subprocess."""
    try:
        from mlpcopilot.config.paths import get_logs_dir

        log_dir = get_logs_dir() / "mcp"
        log_dir.mkdir(parents=True, exist_ok=True)
        path = log_dir / f"{_sanitize_name(server_name)}.stderr.log"
        return stack.enter_context(open(path, "a", encoding="utf-8", buffering=1))
    except Exception as exc:  # pragma: no cover - defensive fallback
        logger.debug("MCP server '{}': failed to open stderr log file: {}", server_name, exc)
        return stack.enter_context(open(os.devnull, "w", encoding="utf-8"))


async def _enter_stdio_client(stack: AsyncExitStack, stdio_client, params, errlog):
    """Enter stdio_client with stderr redirection when the SDK supports it."""
    try:
...
def _annotation_bool(annotations: Any, *names: str) -> bool | None:
...
def _mcp_tool_read_only(tool_def: Any) -> bool:
...
class MCPToolWrapper(Tool):
    """Wraps a single MCP server tool as a mlpcopilot Tool."""
    ...
    def __init__(self, session, server_name: str, tool_def, tool_timeout: int = 120):
        ...
        self._read_only = _mcp_tool_read_only(tool_def)
...
class MCPResourceWrapper(Tool):
    """Wraps an MCP resource URI as a read-only mlpcopilot Tool."""

    def __init__(self, session, server_name: str, resource_def, resource_timeout: int = 120):
        self._session = session
...
class MCPPromptWrapper(Tool):
    """Wraps an MCP prompt as a read-only mlpcopilot Tool."""

    def __init__(self, session, server_name: str, prompt_def, prompt_timeout: int = 120):
        self._session = session
...
async def connect_mcp_servers(
    ...
    The MCP SDK may bind transport cancel scopes to the current asyncio task,
    so stacks are entered sequentially in the caller task and later closed by
    ...
    async def connect_single_server(name: str, cfg) -> tuple[str, AsyncExitStack | None]:
        ...
        try:
            ...
            if transport_type == "stdio":
                ...
                errlog = _open_stdio_errlog(server_stack, name)
                read, write = await _enter_stdio_client(server_stack, stdio_client, params, errlog)
    ...
    for name, cfg in mcp_servers.items():
        try:
            ...
            logger.error("MCP server '{}' connection task failed: {}", name, exc)
            continue
