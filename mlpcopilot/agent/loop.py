...
from mlpcopilot.agent.autocompact import AutoCompact
...
from mlpcopilot.agent.memory import Consolidator, Dream
from mlpcopilot.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
from mlpcopilot.agent.skills import BUILTIN_SKILLS_DIR
from mlpcopilot.agent.subagent import SubagentManager
from mlpcopilot.agent.tools.ask import (
    AskUserTool,
...
from mlpcopilot.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
from mlpcopilot.agent.tools.filesystem import (
    EditFileTool,
    FileInfoTool,
    ListDirTool,
    ReadFileTool,
...
from mlpcopilot.agent.tools.notebook import NotebookEditTool
from mlpcopilot.agent.tools.registry import ToolRegistry
from mlpcopilot.agent.tools.search import GlobTool, GrepTool
from mlpcopilot.agent.tools.self import MyTool
...
from mlpcopilot.agent.tools.spawn import SpawnTool
from mlpcopilot.agent.tools.web import WebFetchTool, WebSearchTool
...
from mlpcopilot.command import CommandContext, CommandRouter, register_builtin_commands
from mlpcopilot.config.schema import AgentDefaults
from mlpcopilot.providers.base import LLMProvider
from mlpcopilot.providers.factory import ProviderSnapshot
from mlpcopilot.runtime.workstate import format_workstate_context
from mlpcopilot.session.manager import Session, SessionManager
from mlpcopilot.utils.document import extract_documents
from mlpcopilot.utils.helpers import image_placeholder_text
from mlpcopilot.utils.helpers import truncate_text as truncate_text_fn
from mlpcopilot.utils.progress_events import (
...
if TYPE_CHECKING:
    from mlpcopilot.config.schema import ChannelsConfig, ExecToolConfig, ToolsConfig, WebToolsConfig
...
class AgentLoop:
    ...
    def __init__(
        ...
        from mlpcopilot.config.schema import ExecToolConfig, ToolsConfig, WebToolsConfig
        ...
        self._tool_approval_allowlist = tuple(_tc.approval_allowlist)
        ...
        # MLPCOPILOT_MAX_CONCURRENT_REQUESTS: <=0 means unlimited; default 3.
        _max = int(os.environ.get("MLPCOPILOT_MAX_CONCURRENT_REQUESTS", "3"))
        ...
        dream_cfg = getattr(getattr(getattr(runtime_config, "agents", None), "defaults", None), "dream", None)
        self.dream = Dream(
            store=self.context.memory,
            provider=provider,
            model=getattr(dream_cfg, "model_override", None) or self.model,
            max_batch_size=getattr(dream_cfg, "max_batch_size", 20),
            max_iterations=getattr(dream_cfg, "max_iterations", 10),
            annotate_line_ages=getattr(dream_cfg, "annotate_line_ages", True),
            approval_required=getattr(dream_cfg, "approval_required", False),
        )
        self._register_default_tools()
        if _tc.my.enable and self._builtin_tool_enabled("my"):
    ...
    def _builtin_tool_enabled(self, name: str) -> bool:
        return self._enabled_builtin_tools is None or name in self._enabled_builtin_tools
    ...
    def switch_runtime_model(self, model: str) -> str:
        """Switch the in-process runtime model for future turns."""
        ...
        if self.runtime_config is None:
            return "Error: active runtime cannot switch models without config context"

        from mlpcopilot.providers.factory import build_provider_snapshot

        updated = self.runtime_config.model_copy(deep=True)
        updated.agents.defaults.model = new_model
        snapshot = build_provider_snapshot(updated)
        self._apply_provider_snapshot(snapshot)
        self.runtime_config.agents.defaults.model = new_model
        self._manual_model_override = new_model
    ...
    def _refresh_provider_snapshot(self) -> None:
        ...
        try:
            ...
            return
        if self._manual_model_override and self.runtime_config is not None:
            from mlpcopilot.providers.factory import build_provider_snapshot

            updated = self.runtime_config.model_copy(deep=True)
            updated.agents.defaults.model = self._manual_model_override
            snapshot = build_provider_snapshot(updated)
    ...
    def _register_default_tools(self) -> None:
        ...
        if self._builtin_tool_enabled("file_info"):
            self.tools.register(
                FileInfoTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
        ...
        if self._builtin_tool_enabled("write_file"):
            self.tools.register(
                WriteFileTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
                    write_allowlist=self._write_allowlist,
                    approval_gated_writes=self._approval_gated_writes,
                )
            )
        if self._builtin_tool_enabled("edit_file"):
            self.tools.register(
                EditFileTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
        ...
        if self._builtin_tool_enabled("list_dir"):
            self.tools.register(
                ListDirTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
        ...
        if self._builtin_tool_enabled("glob"):
            self.tools.register(
                GlobTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
        ...
        if self._builtin_tool_enabled("grep"):
            self.tools.register(
                GrepTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
        ...
        if self._builtin_tool_enabled("notebook_edit"):
            self.tools.register(
                NotebookEditTool(
                    workspace=self.workspace,
                    allowed_dir=allowed_dir,
        ...
        if self.exec_config.enable and self._builtin_tool_enabled("exec"):
            self.tools.register(
                ExecTool(
                    ...
                    allow_commands=self.exec_config.allow_commands,
                    readonly_commands=self.exec_config.readonly_commands,
                    background_commands=self.exec_config.background_commands,
                    allow_patterns=self.exec_config.allow_patterns,
                    require_allowlist=self.exec_config.require_allowlist,
                    approval_required=self.exec_config.approval_required,
                    path_append=self.exec_config.path_append,
                    allowed_env_keys=self.exec_config.allowed_env_keys,
                )
            )
        if self.web_config.enable and self._builtin_tool_enabled("web_search"):
            self.tools.register(
                WebSearchTool(
        ...
        if self.web_config.enable and self._builtin_tool_enabled("web_fetch"):
            self.tools.register(
                WebFetchTool(
        ...
        if self._builtin_tool_enabled("message"):
            self.tools.register(MessageTool(send_callback=self.bus.publish_outbound, workspace=self.workspace))
        if self._builtin_tool_enabled("workstate"):
            self.tools.register(WorkStateTool(sessions=self.sessions))
        if self._builtin_tool_enabled("spawn"):
            self.tools.register(SpawnTool(manager=self.subagents))
        if self.cron_service and self._builtin_tool_enabled("cron"):
            self.tools.register(
    ...
    async def _connect_mcp(self) -> None:
        ...
        try:
            ...
            self._mcp_errors = {
                name: self._mcp_failure_message(name, cfg)
            ...
            self._mcp_errors = {
                name: "Connection cancelled; it will be retried on the next message."
            ...
            self._mcp_stacks.clear()
            self._mcp_errors = {
                name: f"Connection failed before server status was available: {type(e).__name__}: {e}"
                for name in self._mcp_servers
            }
        finally:
            self._mcp_connecting = False

    def _mcp_failure_message(self, name: str, cfg: Any) -> str:
        ...
        if not transport_type:
            if command:
                ...
                transport_type = "sse" if str(url).rstrip("/").endswith("/sse") else "streamableHttp"
            else:
                return f"{name}: no command or url configured."
        if transport_type == "stdio":
            target = command or "(missing command)"
            return (
                f"{name}: stdio command failed to start or initialize: {target}. "
                "Check command path, args/env, and stderr logs."
            )
        if transport_type in {"sse", "streamableHttp"}:
            target = url or "(missing url)"
            return (
                f"{name}: {transport_type} connection failed: {target}. "
                "Check endpoint reachability, auth headers, proxy/network, and timeout."
            )
        return f"{name}: unsupported MCP transport '{transport_type}'."

    def mcp_status(self) -> dict[str, Any]:
        """Return display-friendly MCP connection state for runtime UIs."""
        configured = sorted(self._mcp_servers.keys())
        connected = sorted(self._mcp_stacks.keys())
        if not configured:
            state = "none"
        elif self._mcp_connecting:
            state = "connecting"
        elif len(connected) == len(configured):
            state = "connected"
        elif connected:
            state = "partial"
        else:
        ...
        return {
            ...
            "errors": [
                {"server": name, "message": self._mcp_errors[name]}
                for name in sorted(self._mcp_errors)
            ],
        }
    ...
    def _set_tool_context(
        ...
        for name in ("message", "spawn", "cron", "my", "workstate"):
    ...
    def _tool_hint(tool_calls: list) -> str:
        ...
        from mlpcopilot.utils.tool_hints import format_tool_hints
    ...
    async def _run_agent_loop(
        ...
        *on_stream_end(resuming)*: called when a streaming session finishes.
        ...
        session_key_token = bind_session_key(active_session_key)
        try:
            result = await self.runner.run(AgentRunSpec(
                ...
                approval_required_for_tools=self._approval_required_for_tools,
                tool_approval_allowlist=self._tool_approval_allowlist,
            ))
        finally:
            reset_session_key(session_key_token)
    ...
    async def _process_message(
        ...
        if msg.channel == "system":
            ...
            messages = self.context.build_messages(
                ...
                workstate_context=format_workstate_context(session),
            )
        ...
        skip_user_persist = bool(msg.metadata.get("_skip_user_persist"))
        media_paths = [p for p in (msg.media or []) if isinstance(p, str) and p]
        has_text = isinstance(msg.content, str) and msg.content.strip()
        if not skip_user_persist and not pending_ask_id and (has_text or media_paths):
        ...
        save_skip = 1 + len(history) + (1 if user_persisted_early or skip_user_persist else 0)
        ...
        try:
            ...
            try:
                ...
                if self._restore_runtime_checkpoint(session):
                    ...
                    self.sessions.save(session)
                    logger.info(
                        "Restored partial context for cancelled direct session {}",
                        session_key,
                    )
            except Exception:
                logger.debug(
                    "Could not restore checkpoint for cancelled direct session {}",
                    session_key,
                    exc_info=True,
                )
            raise
