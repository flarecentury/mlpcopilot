...
from mlpcopilot.utils.gitstore import GitStore
...
class Dream:
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        from mlpcopilot.agent.tools.filesystem import EditFileTool, ReadFileTool, WriteFileTool
    ...
    def _batch_approval_metadata(self, batch: list[dict[str, Any]]) -> dict[str, Any]:
        return {
            "tool": "dream",
            ...
            "risk_level": "high",
            "risk_reason": "Dream can mutate long-term memory and workspace skills",
        }

    @staticmethod
    def _same_dream_batch(record_metadata: dict[str, Any], batch_metadata: dict[str, Any]) -> bool:
        return (
            record_metadata.get("tool") == "dream"
            and record_metadata.get("operation") == "memory_update"
            and record_metadata.get("history_start_cursor") == batch_metadata["history_start_cursor"]
            and record_metadata.get("history_end_cursor") == batch_metadata["history_end_cursor"]
        )

    def _dream_approval_request(
        self,
        *,
        ...
        history_preview = "\n".join(
            f"- {truncate_text(e['content'], 240)}"
            for e in batch[:5]
        )
        if len(batch) > 5:
            history_preview += f"\n- ... {len(batch) - 5} more entrie(s)"
        return (
            "Dream analyzed conversation history and wants permission to update "
            "long-term memory and/or workspace skills.\n\n"
            f"History cursors: {batch[0]['cursor']} -> {batch[-1]['cursor']}\n\n"
            f"History preview:\n{history_preview}\n\n"
            f"Analysis preview:\n{truncate_text(analysis.strip() or '(empty)', 1200)}"
        )

    def _dream_approval_allows_phase2(self, batch: list[dict[str, Any]], analysis: str) -> bool:
        from mlpcopilot.runtime.approval import ApprovalManager

        manager = ApprovalManager(self.store.workspace)
        metadata = self._batch_approval_metadata(batch)
        for record in manager.list_pending():
            if self._same_dream_batch(record.metadata or {}, metadata):
                self.last_status = "approval_pending"
                logger.info("Dream waiting for approval {}", record.approval_id)
                return False
        for record in reversed(manager.list_decisions()):
            if not self._same_dream_batch(record.metadata or {}, metadata):
                continue
            if record.status in {"approved", "partially_approved"}:
                self.last_status = "approval_approved"
                return True
            self.last_status = f"approval_{record.status}"
            logger.info("Dream blocked by {} approval {}", record.status, record.approval_id)
            return False

        record = manager.create(
            action_type="memory_update",
            title="Approve Dream memory update",
            request=self._dream_approval_request(batch=batch, analysis=analysis),
            requester="dream",
            metadata=metadata,
        )
        self.last_status = "approval_pending"
        logger.info("Dream requested approval {}", record.approval_id)
        return False

    def _dream_approval_blocks_before_analysis(self, batch: list[dict[str, Any]]) -> bool:
        from mlpcopilot.runtime.approval import ApprovalManager
    ...
    async def run(self) -> bool:
        ...
        if self.approval_required and self._dream_approval_blocks_before_analysis(batch):
            return False
        ...
        if self.approval_required and not self._dream_approval_allows_phase2(batch, analysis):
            return False
