...
class GitStore:
    ...
    def init(self, *, silent: bool = False) -> bool:
        ...
        try:
            ...
            porcelain.commit(
                str(self._workspace),
                message=b"init: mlpcopilot memory store",
                author=b"mlpcopilot <mlpcopilot@dream>",
                committer=b"mlpcopilot <mlpcopilot@dream>",
            )
    ...
    def _is_inside_git_repo(self) -> bool:
        ...
        if any parent directory contains a valid .git entry.
        ...
        while current != current.parent:
            if self._valid_git_entry(current / ".git"):
                return True
            current = current.parent
        return False

    def _valid_git_entry(self, path: Path) -> bool:
        """Return True when a .git path looks like a real git repo marker."""
        if path.is_dir():
        ...
        if not first_line.lower().startswith(prefix):
            return False
        target = first_line[len(prefix):].strip()
        if not target:
            return False
        gitdir = Path(target)
        if not gitdir.is_absolute():
            gitdir = path.parent / gitdir
