...
class TestLineAges:
    ...
    def test_age_differentiates_across_days(self, git, tmp_path):
        ...
        with patch("mlpcopilot.utils.gitstore.datetime") as mock_dt:
...
class TestNestedRepoProtection:
    ...
    def test_init_ignores_invalid_parent_git_marker(self, tmp_path):
        """An unrelated empty .git marker should not poison temp workspaces."""
        project = tmp_path / "project"
        project.mkdir()
        (project / ".git").mkdir()
