...
from mlpcopilot.config.paths import (
...
def test_runtime_dirs_follow_config_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("mlpcopilot.config.paths.get_config_path", lambda: config_file)
...
def test_shared_and_legacy_paths_remain_global() -> None:
    assert get_cli_history_path() == Path.home() / ".mlpcopilot" / "history" / "cli_history"
    assert get_bridge_install_dir() == Path.home() / ".mlpcopilot" / "bridge"
    assert get_legacy_sessions_dir() == Path.home() / ".mlpcopilot" / "sessions"


def test_workspace_path_is_explicitly_resolved(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("HOME", str(tmp_path))

    assert get_workspace_path() == tmp_path / ".mlpcopilot" / "workspace"
...
def test_is_default_workspace_distinguishes_default_and_custom_paths(
    ...
    assert is_default_workspace(tmp_path / ".mlpcopilot" / "workspace") is True
