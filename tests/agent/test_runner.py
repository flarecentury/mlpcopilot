...
from mlpcopilot.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("mlpcopilot.agent.loop.ContextBuilder"), \
         patch("mlpcopilot.agent.loop.SessionManager"), \
         patch("mlpcopilot.agent.loop.SubagentManager") as mock_sub_mgr:
...
@pytest.mark.asyncio
async def test_runner_pauses_when_tool_requires_runtime_approval(tmp_path):
        ...
        LLMResponse(
            ...
            tool_calls=[
                ToolCallRequest(
                    id="call_1",
                    name="list_dir",
                    arguments={"path": str(tmp_path), "recursive": True},
                )
            ],
        ),
    ...
    tools.execute = AsyncMock(return_value="should not execute")

    result = await AgentRunner(provider).run(AgentRunSpec(
        initial_messages=[{"role": "user", "content": "inspect workspace"}],
        tools=tools,
        model="test-model",
        max_iterations=5,
    ...
    assert result.messages[-1]["name"] == "list_dir"
    pending = ApprovalManager(tmp_path, session_key="tui:test").list_pending()
    assert len(pending) == 1
    assert pending[0].metadata["tool"] == "list_dir"


@pytest.mark.asyncio
async def test_runner_registers_same_turn_approvals_then_pauses(tmp_path):
    from mlpcopilot.agent.runner import AgentRunner, AgentRunSpec
    from mlpcopilot.runtime.approval import ApprovalManager
    ...
    provider.chat_with_retry = AsyncMock(side_effect=[
        LLMResponse(
            ...
            finish_reason="tool_calls",
            tool_calls=[
                ToolCallRequest(id="call_1", name="list_dir", arguments={"path": str(tmp_path)}),
                ToolCallRequest(id="call_2", name="glob", arguments={"pattern": "**/*metrics*"}),
            ],
        ),
        LLMResponse(content="should not continue"),
    ])
    ...
    result = await AgentRunner(provider).run(AgentRunSpec(
        initial_messages=[{"role": "user", "content": "find metrics"}],
        tools=tools,
        model="test-model",
        max_iterations=5,
        max_tool_result_chars=_MAX_TOOL_RESULT_CHARS,
    ...
    tools.execute.assert_not_awaited()
    assert result.stop_reason == "approval_required"
    assert [event["status"] for event in result.tool_events] == ["waiting", "waiting"]
    pending = ApprovalManager(tmp_path, session_key="tui:test").list_pending()
    assert [record.metadata["tool"] for record in pending] == ["list_dir", "glob"]
...
async def test_runner_calls_hooks_in_order():
    from mlpcopilot.agent.hook import AgentHook, AgentHookContext
    from mlpcopilot.agent.runner import AgentRunner, AgentRunSpec
...
async def test_runner_persists_large_tool_results_for_follow_up_calls(tmp_path):
    ...
    assert (tmp_path / ".mlpcopilot" / "tool-results" / "test_runner" / "call_big.txt").exists()


def test_persist_tool_result_prunes_old_session_buckets(tmp_path):
    from mlpcopilot.utils.helpers import maybe_persist_tool_result

    root = tmp_path / ".mlpcopilot" / "tool-results"
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "mlpcopilot.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "mlpcopilot.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("mlpcopilot.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "mlpcopilot.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("mlpcopilot.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_llm_error_not_appended_to_session_messages():
    ...
    from mlpcopilot.agent.runner import (
...
async def test_next_turn_after_llm_error_keeps_turn_boundary(tmp_path):
    from mlpcopilot.agent.loop import AgentLoop
    from mlpcopilot.agent.runner import _PERSISTED_MODEL_ERROR_PLACEHOLDER
    from mlpcopilot.bus.events import InboundMessage
    from mlpcopilot.bus.queue import MessageBus
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    from mlpcopilot.agent.subagent import SubagentManager, SubagentStatus
    from mlpcopilot.bus.queue import MessageBus
    ...
    monkeypatch.setattr("mlpcopilot.agent.tools.filesystem.ListDirTool.execute", fake_execute)
...
async def test_length_recovery_gives_up_after_max_retries():
    ...
    from mlpcopilot.agent.runner import _MAX_LENGTH_RECOVERIES, AgentRunner, AgentRunSpec
...
async def test_backfill_missing_tool_results_inserts_error():
    ...
    from mlpcopilot.agent.runner import _BACKFILL_CONTENT, AgentRunner
...
def test_drop_orphan_tool_results_removes_unmatched_tool_messages():
    from mlpcopilot.agent.runner import AgentRunner
...
async def test_backfill_repairs_model_context_without_shifting_save_turn_boundary(tmp_path):
    ...
    from mlpcopilot.agent.loop import AgentLoop
    from mlpcopilot.agent.runner import _BACKFILL_CONTENT
    from mlpcopilot.bus.events import InboundMessage
    from mlpcopilot.bus.queue import MessageBus
...
async def test_runner_backfill_only_mutates_model_context_not_returned_messages():
    ...
    from mlpcopilot.agent.runner import _BACKFILL_CONTENT, AgentRunner, AgentRunSpec
...
async def test_microcompact_replaces_old_tool_results():
    ...
    from mlpcopilot.agent.runner import _MICROCOMPACT_KEEP_RECENT, AgentRunner
...
async def test_injection_cycles_capped_at_max():
    ...
    from mlpcopilot.agent.runner import _MAX_INJECTION_CYCLES, AgentRunner, AgentRunSpec
    from mlpcopilot.bus.events import InboundMessage
...
async def test_pending_queue_preserves_overflow_for_next_injection_cycle(tmp_path):
    ...
    from mlpcopilot.agent.loop import AgentLoop
    from mlpcopilot.agent.runner import _MAX_INJECTIONS_PER_TURN
    from mlpcopilot.bus.events import InboundMessage
    from mlpcopilot.bus.queue import MessageBus
...
async def test_drain_injections_on_empty_final_response():
    ...
    from mlpcopilot.agent.runner import _MAX_EMPTY_RETRIES, AgentRunner, AgentRunSpec
    from mlpcopilot.bus.events import InboundMessage
...
async def test_drain_injections_set_flag_when_followup_arrives_after_last_iteration():
    ...
    from mlpcopilot.agent.hook import AgentHook
    from mlpcopilot.agent.runner import AgentRunner, AgentRunSpec
    from mlpcopilot.bus.events import InboundMessage
...
def test_snip_history_preserves_user_message_after_truncation(monkeypatch):
    ...
    messages = [
        ...
        {"role": "user", "content": ".mlpcopilot的同目录"},
        {
            "role": "assistant",
            "content": None,
    ...
    monkeypatch.setattr("mlpcopilot.agent.runner.estimate_prompt_tokens_chain", lambda *_a, **_kw: (500, None))
