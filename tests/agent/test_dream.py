...
class TestDreamRun:
    ...
    async def test_approval_required_blocks_phase2_until_approved(
        self, dream, mock_provider, mock_runner, store,
    ):
        """Dream should request approval before mutating long-term memory."""
        store.append_history("Remember the active model checkpoint")
        dream.approval_required = True
        mock_provider.chat_with_retry.return_value = MagicMock(content="Store checkpoint fact")
        mock_runner.run = AsyncMock(return_value=_make_run_result())

        result = await dream.run()

        assert result is False
        assert dream.last_status == "approval_pending"
        mock_runner.run.assert_not_called()
        assert store.get_last_dream_cursor() == 0
        pending = ApprovalManager(store.workspace).list_pending()
        assert len(pending) == 1
        assert pending[0].action_type == "memory_update"
        assert pending[0].metadata["tool"] == "dream"
        assert pending[0].metadata["history_start_cursor"] == 1
        assert pending[0].metadata["history_end_cursor"] == 1

    async def test_approved_dream_memory_update_runs_phase2(
        self, dream, mock_provider, mock_runner, store,
    ):
        """An approved Dream memory update should run Phase 2 for the same batch."""
        store.append_history("Remember the active model checkpoint")
        dream.approval_required = True
        mock_provider.chat_with_retry.return_value = MagicMock(content="Store checkpoint fact")
        mock_runner.run = AsyncMock(return_value=_make_run_result())
        ...
        await dream.run()
        pending = ApprovalManager(store.workspace).list_pending()
        ApprovalManager(store.workspace).approve(pending[0].approval_id, decided_by="test")

        result = await dream.run()

        assert result is True
        assert dream.last_status == "completed"
        mock_runner.run.assert_called_once()
        assert store.get_last_dream_cursor() == 1

    async def test_pending_dream_approval_skips_repeated_phase1(
        self, dream, mock_provider, mock_runner, store,
    ):
        """A pending Dream approval should block before spending another LLM call."""
        store.append_history("Remember the active model checkpoint")
        dream.approval_required = True
        mock_provider.chat_with_retry.return_value = MagicMock(content="Store checkpoint fact")
        mock_runner.run = AsyncMock(return_value=_make_run_result())
    ...
    def test_dream_prompts_exclude_live_dpgen_status_from_durable_memory(self):
        phase1 = render_template("agent/dream_phase1.md", stale_threshold_days=14)
        phase2 = render_template("agent/dream_phase2.md", skill_creator_path="skills/skill-creator/SKILL.md")

        assert "Do not promote live DP-GEN status" in phase1
        assert "current iteration" in phase1
        assert "Do not write live DP-GEN status" in phase2
    ...
    async def test_phase1_annotates_only_memory_not_soul_or_user(self, dream, mock_provider, mock_runner, store):
        ...
        assert "Project X active" in memory_section
...
class TestDreamPromptCaps:
    ...
    async def test_phase1_caps_huge_history_entry(
        ...
        # record that was written by an older mlpcopilot build before any caps.
