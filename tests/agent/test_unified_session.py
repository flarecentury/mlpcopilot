...
from mlpcopilot.command.builtin import cmd_new, register_builtin_commands
from mlpcopilot.command.router import CommandContext, CommandRouter
from mlpcopilot.config.schema import AgentDefaults, Config
...
def _make_loop(tmp_path: Path, unified_session: bool = False) -> AgentLoop:
    ...
    with patch("mlpcopilot.agent.loop.SessionManager"), \
         patch("mlpcopilot.agent.loop.SubagentManager") as mock_sub_mgr, \
         patch("mlpcopilot.agent.loop.Dream"):
...
class TestUnifiedSessionConfig:
    ...
    def test_onboard_generated_config_contains_unified_session(self, tmp_path: Path):
        """save_config() writes 'unifiedSession' into config.json (simulates mlpcopilot onboard)."""
        from mlpcopilot.config.loader import save_config
...
class TestConsolidationUnaffectedByUnifiedSession:
    ...
    async def test_consolidation_skips_empty_session_for_unified_key(self):
        ...
        from mlpcopilot.agent.memory import Consolidator, MemoryStore
