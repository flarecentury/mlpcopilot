...
def test_bootstrap_files_are_backed_by_templates() -> None:
    template_dir = pkg_files("mlpcopilot") / "templates"
...
def test_runtime_context_includes_workstate_when_provided(tmp_path) -> None:
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)
    ...
    messages = builder.build_messages(
        ...
        workstate_context=(
            ...
            "Current Plan:\n1. [in_progress] add context injection"
        ),
    )

    user_content = messages[-1]["content"]
    assert isinstance(user_content, str)
    assert "[Session Work State]" in user_content
    assert "Current Goal:\nfinish runtime plan support" in user_content
    assert "1. [in_progress] add context injection" in user_content


def test_runtime_context_includes_active_project_pointer(tmp_path) -> None:
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)

    messages = builder.build_messages(
        history=[],
        current_message="Check status",
        channel="cli",
        chat_id="direct",
        workstate_context=(
            "[Active MLP Project]\n"
            ...
            "run_id: run_local\n"
            "backend: dpgen\n"
            "project_path: /tmp/project/backend/dpgen\n"
    ...
    user_content = messages[-1]["content"]
    assert isinstance(user_content, str)
    assert "[Active MLP Project]" in user_content
    assert "project_id: local_dpgen" in user_content
    assert "status_source: call MCP tools for live state" in user_content
...
def test_execution_rules_in_system_prompt(tmp_path) -> None:
    ...
    assert "persist it with the `workstate` tool" in prompt


def test_identity_has_stable_runtime_context_rules(tmp_path) -> None:
    """Identity template should keep runtime context guidance without legacy identity text."""
    workspace = _make_workspace(tmp_path)
    builder = ContextBuilder(workspace)

    identity = builder._get_identity(channel=None)
    assert "You are mlpcopilot" not in identity
    assert "Act, don't narrate" not in identity
    assert "Execution Rules" not in identity
    assert "[Session Work State]" in identity
    assert "refresh live MLP/DP-GEN status" in identity
...
def test_default_soul_template_contains_execution_rules() -> None:
    ...
    soul = (pkg_files("mlpcopilot") / "templates" / "SOUL.md").read_text(encoding="utf-8")
...
def test_always_skills_excluded_from_skills_index(tmp_path) -> None:
    ...
    assert "### Skill: agentic-file-search" in prompt
    ...
    if len(skills_section) > 1:
        ...
        assert "**agentic-file-search**" not in index_text
...
def test_template_memory_md_is_skipped(tmp_path) -> None:
    ...
    assert "This file is automatically updated by mlpcopilot" not in prompt


def test_template_user_md_is_skipped(tmp_path) -> None:
    """USER.md matching the bundled template should not consume prompt context."""
    workspace = _make_workspace(tmp_path)
    from mlpcopilot.utils.helpers import sync_workspace_templates
    sync_workspace_templates(workspace, silent=True)
    ...
    assert "Information about the user to help personalize interactions" not in prompt


def test_customized_user_md_is_injected(tmp_path) -> None:
    workspace = _make_workspace(tmp_path)
    from mlpcopilot.utils.helpers import sync_workspace_templates
    sync_workspace_templates(workspace, silent=True)
