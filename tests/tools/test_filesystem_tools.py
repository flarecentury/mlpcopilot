...
class TestEditFileTool:
    ...
    @pytest.mark.asyncio
    async def test_edit_blocks_runtime_managed_memory_history(self, tmp_path: Path):
        history = tmp_path / "memory" / "history.jsonl"
        history.parent.mkdir(parents=True)
        history.write_text('{"content": "old"}\n', encoding="utf-8")
        ...
        assert "runtime-managed append-only state" in result
        assert history.read_text(encoding="utf-8") == '{"content": "old"}\n'
...
class TestWorkspaceRestriction:
    ...
    async def test_read_allowed_in_media_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mlpcopilot.agent.tools.filesystem.get_media_dir", lambda: media_dir)
    ...
    async def test_extra_dirs_does_not_widen_write(self, tmp_path):
        from mlpcopilot.agent.tools.filesystem import WriteFileTool
