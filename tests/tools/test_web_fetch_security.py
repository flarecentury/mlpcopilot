...
from mlpcopilot.agent.tools.web import WebFetchTool
...
async def test_web_fetch_blocks_localhost():
    ...
    with patch("mlpcopilot.security.network.socket.getaddrinfo", _resolve_localhost):
...
async def test_web_fetch_result_contains_untrusted_flag():
    ...
    with patch("mlpcopilot.security.network.socket.getaddrinfo", _fake_resolve_public), \
         patch("mlpcopilot.agent.tools.web.httpx.AsyncClient", FakeClient):
...
async def test_web_fetch_can_skip_jina_and_use_custom_user_agent(monkeypatch):
    tool = WebFetchTool(
        ...
        user_agent="mlpcopilot-test-agent",
    )
    ...
    monkeypatch.setattr("mlpcopilot.agent.tools.web.httpx.AsyncClient", FakeClient)
