...
from mlpcopilot.agent.tools import (
    ArraySchema,
    IntegerSchema,
    ObjectSchema,
    Schema,
...
def test_exec_extract_absolute_paths_captures_home_paths() -> None:
    cmd = "cat ~/.mlpcopilot/config.json > ~/out.txt"
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "~/.mlpcopilot/config.json" in paths
    assert "~/out.txt" in paths


def test_exec_extract_absolute_paths_captures_quoted_paths() -> None:
    cmd = 'cat "/tmp/data.txt" "~/.mlpcopilot/config.json"'
    paths = ExecTool._extract_absolute_paths(cmd)
    assert "/tmp/data.txt" in paths
    assert "~/.mlpcopilot/config.json" in paths
...
def test_exec_guard_blocks_home_path_outside_workspace(tmp_path) -> None:
    ...
    error = tool._guard_command("cat ~/.mlpcopilot/config.json", str(tmp_path))
...
def test_exec_guard_allows_media_path_outside_workspace(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("mlpcopilot.agent.tools.shell.get_media_dir", lambda: media_dir)
...
def test_exec_guard_blocks_windows_drive_root_outside_workspace(monkeypatch) -> None:
    import mlpcopilot.agent.tools.shell as shell_mod
...
async def test_exec_head_tail_truncation(tmp_path) -> None:
    ...
    assert "Full output: jobs/" in result
    assert "chars truncated" in result
    # Head and tail preview should still preserve representative output.
