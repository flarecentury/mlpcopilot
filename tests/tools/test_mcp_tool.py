...
import mlpcopilot.agent.tools.mcp as mcp_mod
from mlpcopilot.agent.tools.mcp import (
    MCPPromptWrapper,
    MCPResourceWrapper,
    MCPToolWrapper,
...
def test_mcp_server_config_default_tool_timeout_is_120() -> None:
    assert MCPServerConfig(command="fake").tool_timeout == 120
...
def test_tool_wrapper_defaults_to_not_read_only_without_annotations() -> None:
    wrapper = _make_wrapper(SimpleNamespace(call_tool=None))

    assert wrapper.read_only is False


def test_tool_wrapper_read_only_follows_standard_mcp_annotations() -> None:
    tool_def = SimpleNamespace(
        name="status",
...
def test_tool_wrapper_destructive_annotation_wins_over_read_only_hint() -> None:
    tool_def = SimpleNamespace(
        name="confused",
        description="confused tool",
        inputSchema={"type": "object", "properties": {}},
...
@pytest.mark.asyncio
async def test_connect_mcp_servers_supports_sse_transport(
    fake_mcp_runtime: dict[str, object | None],
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session(["remote_tool"])
    registry = ToolRegistry()

    stacks = await connect_mcp_servers(
        {"remote": MCPServerConfig(type="sse", url="http://mcp.example.test/sse")},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()

    assert registry.tool_names == ["mcp_remote_remote_tool"]
    assert set(stacks) == {"remote"}


@pytest.mark.asyncio
async def test_connect_mcp_servers_supports_streamable_http_transport(
    fake_mcp_runtime: dict[str, object | None],
) -> None:
    fake_mcp_runtime["session"] = _make_fake_session(["remote_tool"])
    registry = ToolRegistry()
    ...
    stacks = await connect_mcp_servers(
        {"remote": MCPServerConfig(type="streamableHttp", url="http://mcp.example.test/mcp")},
        registry,
    )
    for stack in stacks.values():
        await stack.aclose()
...
async def test_connect_mcp_servers_enabled_tools_warns_on_unknown_entries(
    ...
    monkeypatch.setattr("mlpcopilot.agent.tools.mcp.logger.warning", _warning)
...
async def test_connect_mcp_servers_logs_stdio_pollution_hint(
    ...
    monkeypatch.setattr("mlpcopilot.agent.tools.mcp.logger.error", _error)
