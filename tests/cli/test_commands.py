...
from mlpcopilot.cli.commands import _make_provider, app
from mlpcopilot.config.schema import Config
from mlpcopilot.cron.types import CronJob, CronPayload
from mlpcopilot.providers.factory import ProviderSnapshot
from mlpcopilot.providers.openai_codex_provider import _strip_model_prefix
...
def mock_paths():
    ...
    with patch("mlpcopilot.config.loader.get_config_path") as mock_cp, \
         patch("mlpcopilot.config.loader.save_config") as mock_sc, \
         patch("mlpcopilot.config.loader.load_config") as mock_lc, \
         patch("mlpcopilot.cli.onboard_commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "MLP Copilot is ready" in result.stdout
...
def test_root_help_uses_product_name_once():
    result = runner.invoke(app, ["--help"])

    assert result.exit_code == 0
    stripped_output = _strip_ansi(result.stdout)
    assert "MLP Copilot - MLP workflow runtime" in stripped_output
    assert "MLP MLP Copilot" not in stripped_output


def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    config_file, workspace_dir, _ = mock_paths

    from mlpcopilot.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "mlpcopilot.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mlpcopilot.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'mlpcopilot agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"mlpcopilot gateway --config {resolved_config}" in compact_output
...
def test_mlp_init_creates_profile_config_and_workspace(tmp_path, monkeypatch):
    from types import SimpleNamespace

    config_path = tmp_path / "instance" / "config.json"
    workspace = tmp_path / "mlp-workspace"
    monkeypatch.setattr(
        "mlpcopilot.channels.registry.discover_all",
        lambda: {
            "telegram": SimpleNamespace(
                default_config=lambda: {"enabled": False, "token": "", "allowFrom": []}
            ),
            "slack": SimpleNamespace(
                default_config=lambda: {"enabled": False, "botToken": "", "allowFrom": []}
            ),
        },
    )
    ...
    result = runner.invoke(
        app,
        ["mlp", "init", "--config", str(config_path), "--workspace", str(workspace)],
    )

    assert result.exit_code == 0
    saved = json.loads(config_path.read_text(encoding="utf-8"))
    assert saved["runtimeProfile"] == "mlpcopilot"
    ...
    assert saved["tools"]["exec"]["requireAllowlist"] is True
    assert saved["tools"]["exec"]["approvalRequired"] is True
    assert "ls -al" in saved["tools"]["exec"]["allowCommands"]
    assert "ls" in saved["tools"]["exec"]["readonlyCommands"]
    assert set(saved["tools"]["approvalAllowlist"]) >= {
        "read_file",
        "file_info",
        "list_dir",
        "grep",
    ...
    assert saved["tools"]["writeAllowlist"] == []
    assert saved["tools"]["restrictToWorkspace"] is True
    assert saved["tools"]["approvalGatedWrites"] is True
    assert set(saved["channels"]) >= {
        "sendProgress",
        "sendToolHints",
        "sendMaxRetries",
    ...
    assert "slack" not in saved["channels"]
    assert (workspace / "PROJECT.md").exists()
    assert not (workspace / "capabilities").exists()
    assert not (workspace / "capabilities" / "mcp.json").exists()


def test_mlp_approval_and_runs_commands(tmp_path, monkeypatch):
    ...
    approval = ApprovalManager(workspace).create(
        action_type="costly_job",
        title="Run validation",
        request="Launch validation",
    )
    ArtifactIndex(workspace).create_run(
        run_id="run_test",
        source="mcp:test:tool",
        artifacts=["runs/run_test/output.json"],
    )

    approvals = runner.invoke(app, ["mlp", "approvals", "--config", str(config_path)])
    assert approvals.exit_code == 0
    assert approval.approval_id in approvals.stdout

    approved = runner.invoke(app, ["mlp", "approve", approval.approval_id, "--config", str(config_path)])
    assert approved.exit_code == 0
    assert "marked approved" in approved.stdout

    runs = runner.invoke(app, ["mlp", "runs", "list", "--config", str(config_path)])
    assert runs.exit_code == 0
    assert "run_test" in runs.stdout

    shown = runner.invoke(app, ["mlp", "runs", "show", "run_test", "--config", str(config_path)])
    assert shown.exit_code == 0
    assert "mcp:test:tool" in shown.stdout


def test_tui_once_renders_snapshot_from_config(tmp_path):
    config_path = tmp_path / "config.json"
    workspace = tmp_path / "workspace"
    config = Config.model_validate(
        {
            "runtimeProfile": "mlpcopilot",
            "agents": {"defaults": {"workspace": str(workspace), "model": "test/model"}},
            "tools": {"mcpServers": {}},
        }
    )
    config_path.write_text(json.dumps(config.model_dump(by_alias=True)), encoding="utf-8")

    result = runner.invoke(app, ["tui", "--once", "--config", str(config_path)])

    assert result.exit_code == 0
    assert "Chat / Task" in result.stdout
    assert "Tool Log" in result.stdout
    assert "Approvals" in result.stdout
    assert "current model: test/model" in result.stdout


def test_mlp_project_run_dpgen_and_artifact_commands(tmp_path, monkeypatch):
    config_path = tmp_path / "instance" / "config.json"
    workspace = tmp_path / "mlp-workspace"
    monkeypatch.setattr("mlpcopilot.channels.registry.discover_all", lambda: {})
    ...
    assert "proj_cli" in project_result.stdout

    run_result = runner.invoke(
        app,
        ["mlp", "runs", "create", "proj_cli", "--run-id", "run_cli", "--config", str(config_path)],
    )
    assert run_result.exit_code == 0
    backend = workspace / "projects" / "proj_cli" / "runs" / "run_cli" / "backend" / "dpgen"
    backend.mkdir(parents=True, exist_ok=True)
    (backend / "param.json").write_text("{}\n", encoding="utf-8")
    (backend / "machine.json").write_text("{}\n", encoding="utf-8")
    (backend / "record.dpgen").write_text("0 2\n", encoding="utf-8")
    (backend / "iter.000000" / "00.train").mkdir(parents=True)

    sync_result = runner.invoke(
        app,
        ["mlp", "runs", "sync-dpgen", "proj_cli", "run_cli", "--config", str(config_path)],
    )
    assert sync_result.exit_code == 0
    assert "record_entries" in sync_result.stdout

    inspect_result = runner.invoke(
        app,
        ["mlp", "artifacts", "inspect", "proj_cli", "run_cli", "record.dpgen", "--config", str(config_path)],
    )
    assert inspect_result.exit_code == 0
    assert "run_record" in inspect_result.stdout

    attach_result = runner.invoke(
        app,
        ["mlp", "artifacts", "attach", "proj_cli", "run_cli", "record.dpgen", "--config", str(config_path)],
    )
    assert attach_result.exit_code == 0
    assert "context_instruction" in attach_result.stdout

    lineage_result = runner.invoke(
        app,
        ["mlp", "artifacts", "lineage", "proj_cli", "run_cli", "record.dpgen", "--config", str(config_path)],
    )
    assert lineage_result.exit_code == 0
    assert "parents" in lineage_result.stdout


def test_tui_once_renders_runtime_panes(tmp_path, monkeypatch):
    config_path = tmp_path / "instance" / "config.json"
    workspace = tmp_path / "mlp-workspace"
    monkeypatch.setattr("mlpcopilot.channels.registry.discover_all", lambda: {})
    ...
    assert "Chat / Task" in stripped
    assert "Tool Log" in stripped
    assert "Artifacts" in stripped
    assert "Approvals" in stripped
    assert "Enter send" in stripped
    assert "writes" in stripped


def test_tui_starts_without_provider_api_key(tmp_path, monkeypatch):
    from mlpcopilot.runtime import tui as tui_module
    ...
    async def fake_run_tui(config_arg, provider_arg, *, session_id, console, provider_error=None):
        seen["config"] = config_arg
        seen["provider"] = provider_arg
        seen["session_id"] = session_id
        seen["provider_error"] = provider_error

    monkeypatch.setattr("mlpcopilot.cli.tui_commands._load_runtime_config", lambda *a, **k: config)
    monkeypatch.setattr("mlpcopilot.cli.tui_commands._sync_runtime_workspace", lambda *a, **k: [])
    monkeypatch.setattr(tui_module, "run_tui", fake_run_tui)

    result = runner.invoke(app, ["tui", "--session", "tui:test"])

    assert result.exit_code == 0
    assert seen["config"] is config
    assert seen["provider"] is None
    assert seen["session_id"] == "tui:test"
    assert "No API key configured" in seen["provider_error"]
...
def test_openai_compat_provider_passes_model_through():
    from mlpcopilot.providers.openai_compat_provider import OpenAICompatProvider

    with patch("mlpcopilot.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from mlpcopilot.cli.commands import _make_provider
    from mlpcopilot.config.schema import Config
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from mlpcopilot.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("mlpcopilot.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("mlpcopilot.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("mlpcopilot.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("mlpcopilot.config.loader.resolve_config_env_vars", side_effect=lambda c: c), \
         patch("mlpcopilot.runtime.workspace.sync_mlpcopilot_workspace") as mock_sync_templates, \
         patch("mlpcopilot.cli.chat_commands._make_provider", return_value=object()), \
         patch("mlpcopilot.cli.chat_commands._print_agent_response") as mock_print_response, \
         patch("mlpcopilot.bus.queue.MessageBus"), \
         patch("mlpcopilot.cron.service.CronService"), \
         patch("mlpcopilot.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "mlpcopilot.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("mlpcopilot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mlpcopilot.runtime.workspace.sync_mlpcopilot_workspace", lambda _path, **_kwargs: None)
    monkeypatch.setattr("mlpcopilot.cli.chat_commands._make_provider", lambda _config: object())
    monkeypatch.setattr("mlpcopilot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("mlpcopilot.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("mlpcopilot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mlpcopilot.cli.chat_commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("mlpcopilot.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("mlpcopilot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mlpcopilot.runtime.workspace.sync_mlpcopilot_workspace", lambda _path, **_kwargs: None)
    monkeypatch.setattr("mlpcopilot.cli.chat_commands._make_provider", lambda _config: object())
    monkeypatch.setattr("mlpcopilot.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("mlpcopilot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("mlpcopilot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mlpcopilot.cli.chat_commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("mlpcopilot.cli.chat_commands._make_provider", lambda _config: object())
    monkeypatch.setattr("mlpcopilot.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("mlpcopilot.config.paths.get_cron_dir", lambda: legacy_dir)
...
def test_agent_custom_config_workspace_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("mlpcopilot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr(
        "mlpcopilot.cli.chat_commands._print_agent_response", lambda *_args, **_kwargs: None
    )
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("mlpcopilot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mlpcopilot.config.loader.resolve_config_env_vars", lambda c: c)
    def _sync_workspace(path: Path, **_kwargs):
        return (sync_templates or (lambda _path: None))(path)

    monkeypatch.setattr(
        "mlpcopilot.runtime.workspace.sync_mlpcopilot_workspace",
        _sync_workspace,
    )
    monkeypatch.setattr(
        "mlpcopilot.cli.chat_commands._make_provider",
        provider_factory,
    )
    monkeypatch.setattr(
        "mlpcopilot.providers.factory.build_provider_snapshot",
        lambda _config: _test_provider_snapshot(provider_factory(_config), _config),
    )
    monkeypatch.setattr(
        "mlpcopilot.providers.factory.load_provider_snapshot",
        lambda _config_path=None: _test_provider_snapshot(provider_factory(config), config),
    )

    if message_bus is not None:
        monkeypatch.setattr("mlpcopilot.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("mlpcopilot.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("mlpcopilot.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("mlpcopilot.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("mlpcopilot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mlpcopilot.api.server.create_app", _fake_create_app)
...
def test_gateway_cron_evaluator_receives_scheduled_reminder_context(
    ...
    monkeypatch.setattr("mlpcopilot.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("mlpcopilot.runtime.workspace.sync_mlpcopilot_workspace", lambda _path, **_kwargs: None)
    monkeypatch.setattr("mlpcopilot.cli.chat_commands._make_provider", lambda _config: provider)
    monkeypatch.setattr(
        "mlpcopilot.providers.factory.build_provider_snapshot",
    ...
    monkeypatch.setattr("mlpcopilot.bus.queue.MessageBus", lambda: bus)
    ...
    monkeypatch.setattr("mlpcopilot.session.manager.SessionManager", _FakeSessionManager)
    ...
    monkeypatch.setattr("mlpcopilot.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("mlpcopilot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mlpcopilot.channels.manager.ChannelManager", _StopAfterCronSetup)
    monkeypatch.setattr(
        "mlpcopilot.utils.evaluator.evaluate_response",
...
def test_gateway_cron_job_suppresses_intermediate_progress(
    ...
    monkeypatch.setattr("mlpcopilot.bus.queue.MessageBus", lambda: bus)
    monkeypatch.setattr("mlpcopilot.session.manager.SessionManager", lambda _workspace: object())
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from mlpcopilot.cli.commands import _migrate_cron_store
    ...
    with patch("mlpcopilot.config.paths.get_cron_dir", return_value=legacy_dir):
...
def test_gateway_health_endpoint_binds_and_serves_expected_responses(
    ...
    monkeypatch.setattr("mlpcopilot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("mlpcopilot.channels.manager.ChannelManager", _FakeChannelManager)
    monkeypatch.setattr("mlpcopilot.cron.service.CronService", _FakeCronService)
    monkeypatch.setattr("mlpcopilot.heartbeat.service.HeartbeatService", _FakeHeartbeatService)
...
def test_serve_uses_api_config_defaults_and_workspace_override(
    ...
    assert seen["runtime_config"] is config
...
def test_serve_rejects_public_cli_host_without_api_auth(monkeypatch, tmp_path: Path) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    seen: dict[str, object] = {}
    ...
    assert "api.apiKey" in result.stdout
    assert "host" not in seen


def test_serve_allows_public_cli_host_with_proxy_auth(monkeypatch, tmp_path: Path) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    config.api.trust_proxy_auth = True
    seen: dict[str, object] = {}
