...
from mlpcopilot.cli import chat_commands, commands
from mlpcopilot.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("mlpcopilot.cli.chat_commands._PROMPT_SESSION", mock_session), \
         patch("mlpcopilot.cli.chat_commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    chat_commands._PROMPT_SESSION = None

    with patch("mlpcopilot.cli.chat_commands.PromptSession") as mock_session, \
         patch("mlpcopilot.cli.chat_commands.FileHistory") as _mock_history, \
        ...
        assert chat_commands._PROMPT_SESSION is not None
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("mlpcopilot.cli.chat_commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
