...
from mlpcopilot.channels.matrix import _build_matrix_text_content

import mlpcopilot.channels.matrix as matrix_module
from mlpcopilot.bus.events import OutboundMessage
from mlpcopilot.bus.queue import MessageBus
from mlpcopilot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from mlpcopilot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("mlpcopilot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "mlpcopilot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("mlpcopilot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "mlpcopilot.channels.matrix.asyncio.create_task", _fake_create_task
    )
