...
from mlpcopilot.channels.telegram import (
...
class _FakeBot:
    ...
    async def get_me(self):
        self.get_me_calls += 1
        return SimpleNamespace(id=999, username="mlpcopilot_test")
...
class _TelegramTools:
    def __init__(self, exec_tool: _TelegramExecTool | None = None) -> None:
...
async def _dispatch_telegram_runtime_command(msg, *, workspace: Path, tools=None):
    router = CommandRouter()
    register_builtin_commands(router)
    loop = SimpleNamespace(workspace=workspace, tools=tools or _TelegramTools(), runtime_config=None)
    ctx = CommandContext(
        msg=msg,
        session=None,
        key=msg.session_key,
...
async def test_start_creates_separate_pools_with_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mlpcopilot.channels.telegram.HTTPXRequest", _FakeHTTPXRequest)
    monkeypatch.setattr(
        "mlpcopilot.channels.telegram.Application",
...
async def test_send_text_retries_on_timeout() -> None:
    ...
    import mlpcopilot.channels.telegram as tg_mod
...
async def test_on_error_logs_network_issues_as_warning(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mlpcopilot.channels.telegram.logger.warning",
        lambda message, error: recorded.append(("warning", message.format(error))),
    )
    monkeypatch.setattr(
        "mlpcopilot.channels.telegram.logger.error",
...
async def test_send_delta_stream_end_html_expansion_does_not_overflow() -> None:
    ...
    """
    from mlpcopilot.channels.telegram import _markdown_to_telegram_html
...
async def test_send_delta_incremental_edit_splits_oversized_buffer() -> None:
    ...
    from mlpcopilot.channels.telegram import TELEGRAM_MAX_MESSAGE_LEN
...
async def test_send_remote_media_url_after_security_validation(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr("mlpcopilot.channels.telegram.validate_url_target", lambda url: (True, ""))
...
async def test_send_blocks_unsafe_remote_media_url(monkeypatch) -> None:
    ...
    channel._app = _FakeApp(lambda: None)
    monkeypatch.setattr(
        "mlpcopilot.channels.telegram.validate_url_target",
...
async def test_group_policy_mention_accepts_text_mention_and_caches_bot_identity() -> None:
    ...
    await channel._on_message(_make_telegram_update(text="@mlpcopilot_test hi", entities=[mention]), None)
    await channel._on_message(_make_telegram_update(text="@mlpcopilot_test again", entities=[mention]), None)
...
async def test_group_policy_mention_accepts_caption_mention() -> None:
    ...
    await channel._on_message(
        _make_telegram_update(caption="@mlpcopilot_test photo", caption_entities=[mention]),
        None,
    )

    assert len(handled) == 1
    assert handled[0]["content"] == "@mlpcopilot_test photo"
...
async def test_download_message_media_returns_path_when_download_succeeds(
    ...
    monkeypatch.setattr(
        "mlpcopilot.channels.telegram.get_media_dir",
...
async def test_forward_command_preserves_dream_log_args_and_strips_bot_suffix() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream-log@mlpcopilot_test deadbeef", reply_to_message=None)
...
async def test_forward_command_normalizes_telegram_safe_dream_aliases() -> None:
    ...
    channel._handle_message = capture_handle
    update = _make_telegram_update(text="/dream_restore@mlpcopilot_test deadbeef", reply_to_message=None)
...
@pytest.mark.asyncio
async def test_telegram_approvals_command_reaches_runtime_manager(tmp_path: Path) -> None:
    ...
    bus = MessageBus()
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["12345"], group_policy="open"),
        bus,
    )
    channel._app = _FakeApp(lambda: None)
    update = _make_telegram_update(text="/approvals")

    await channel._forward_command(update, None)
    msg = await bus.consume_inbound()
    response = await _dispatch_telegram_runtime_command(msg, workspace=tmp_path)

    assert msg.sender_id == "12345|alice"
    assert msg.content == "/approvals"
    assert response is not None
    assert response.channel == "telegram"
    assert response.chat_id == str(update.message.chat_id)
    assert approval.approval_id in response.content
    assert "Pending approvals" in response.content


@pytest.mark.asyncio
async def test_telegram_approve_command_resumes_runtime_action(tmp_path: Path) -> None:
    approval = ApprovalManager(tmp_path).create(
    ...
    exec_tool = _TelegramExecTool()
    tools = _TelegramTools(exec_tool)
    bus = MessageBus()
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["12345"], group_policy="open"),
        bus,
    ...
    channel._app = _FakeApp(lambda: None)
    update = _make_telegram_update(text=f"/approve {approval.approval_id} ok")

    await channel._forward_command(update, None)
    msg = await bus.consume_inbound()
    response = await _dispatch_telegram_runtime_command(msg, workspace=tmp_path, tools=tools)
    assert response is not None
    await channel.send(response)

    assert ApprovalManager(tmp_path).list_pending() == []
    assert "marked approved" in response.content
    assert "Resumed exec after approval" in response.content
    ...
    assert channel._app.bot.sent_messages
    sent_text = channel._app.bot.sent_messages[-1]["text"]
    assert approval.approval_id in sent_text
    assert "Resumed exec after approval" in sent_text
...
@pytest.mark.asyncio
async def test_telegram_reject_and_changes_commands_update_approval(
    tmp_path: Path,
    command: str,
    expected_status: str,
) -> None:
    approval = ApprovalManager(tmp_path).create(
        ...
        title="Run expensive tool",
        request="Call a gated tool",
    )
    bus = MessageBus()
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["12345"], group_policy="open"),
    ...
    update = _make_telegram_update(text=f"/{command} {approval.approval_id} reason")

    await channel._forward_command(update, None)
    msg = await bus.consume_inbound()
    response = await _dispatch_telegram_runtime_command(msg, workspace=tmp_path)
    decided = ApprovalManager(tmp_path).get(approval.approval_id)

    assert response is not None
    assert decided is not None
    assert decided.status == expected_status
    assert expected_status in response.content
    assert ApprovalManager(tmp_path).list_pending() == []


@pytest.mark.asyncio
async def test_telegram_approval_command_respects_allow_from(tmp_path: Path) -> None:
    approval = ApprovalManager(tmp_path).create(
    ...
    bus = MessageBus()
    channel = TelegramChannel(
        TelegramConfig(enabled=True, token="123:abc", allow_from=["99999"], group_policy="open"),
        bus,
    )
    update = _make_telegram_update(text=f"/approve {approval.approval_id}")
...
def test_strip_md_block_removes_inline_formatting() -> None:
    from mlpcopilot.channels.telegram import _strip_md_block
