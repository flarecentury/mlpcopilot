...
try:
    from mlpcopilot.channels import dingtalk
...
import mlpcopilot.channels.dingtalk as dingtalk_module
from mlpcopilot.bus.queue import MessageBus
from mlpcopilot.channels.dingtalk import DingTalkChannel, DingTalkConfig, MLPCopilotDingTalkHandler


def _fake_validate_url_target(url: str) -> tuple[bool, str]:
    if "127.0.0.1" in url or "localhost" in url:
        return False, "Blocked private test URL"
    return True, ""


def _fake_validate_resolved_url(url: str) -> tuple[bool, str]:
    return _fake_validate_url_target(url)


@pytest.fixture(autouse=True)
def _deterministic_url_validation(monkeypatch) -> None:
    monkeypatch.setattr(dingtalk_module, "validate_url_target", _fake_validate_url_target)
    monkeypatch.setattr(dingtalk_module, "validate_resolved_url", _fake_validate_resolved_url)
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "MLPCopilot Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = MLPCopilotDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/mlpcopilot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/mlpcopilot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mlpcopilot.config.paths.get_media_dir",
