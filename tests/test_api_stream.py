...
def test_sse_error_chunk() -> None:
    raw = _sse_error_chunk("backend failed", status=500)
    payload = json.loads(raw.decode().split("data: ", 1)[1])
...
@pytest.mark.skipif(not HAS_AIOHTTP, reason="aiohttp not installed")
@pytest.mark.asyncio
async def test_stream_empty_final_response_uses_fallback(aiohttp_client) -> None:
    """stream=true should emit a visible fallback when the agent returns empty text."""
    ...
    chunks = [json.loads(line) for line in data_lines[:-1]]
    deltas = [c["choices"][0]["delta"].get("content", "") for c in chunks if "choices" in c]

    assert EMPTY_FINAL_RESPONSE_MESSAGE in deltas
    assert data_lines[-1] == "[DONE]"
...
async def test_streaming_backend_failure_does_not_emit_success_terminator(aiohttp_client) -> None:
    """Backend exceptions should emit an SSE error instead of a silent stream."""
    agent = MagicMock()
    ...
    assert "backend blew up" in body
    assert "[DONE]" in body
