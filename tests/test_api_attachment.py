...
def test_parse_json_content_extracts_text_and_media(monkeypatch, tmp_path) -> None:
    """Parse JSON with text + base64 image saves image and returns paths."""
    media_dir = tmp_path / "media" / "api"
    media_dir.mkdir(parents=True)
    monkeypatch.setattr(
        "mlpcopilot.api.server.get_media_dir",
        lambda channel=None: media_dir if channel == "api" else media_dir.parent,
    )
...
# ---------------------------------------------------------------------------
# extract_documents tests (now in mlpcopilot.utils.document)
# ---------------------------------------------------------------------------
...
def test_extract_documents_skips_extraction_errors(tmp_path, monkeypatch) -> None:
    ...
    import mlpcopilot.utils.document as _doc
    monkeypatch.setattr(
        _doc, "extract_text",
