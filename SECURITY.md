...
If you discover a security vulnerability in mlpcopilot, please report it by:
...
chmod 600 ~/.mlpcopilot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.mlpcopilot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.mlpcopilot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run mlpcopilot with a dedicated user account
...
- Keep authentication data in `~/.mlpcopilot/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade mlpcopilot-ai
```
...
- Subscribe to security advisories for mlpcopilot and its dependencies
...
1. **Isolate the Environment**
   ...
   pip install mlpcopilot-ai
   ```

2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash mlpcopilot
   sudo -u mlpcopilot mlpcopilot gateway
   ```

3. **Set Proper Permissions**
   ```bash
   chmod 700 ~/.mlpcopilot
   chmod 600 ~/.mlpcopilot/config.json
   chmod 700 ~/.mlpcopilot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.mlpcopilot/logs/mlpcopilot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.mlpcopilot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.mlpcopilot/logs/mlpcopilot.log
   ```
...
- GitHub Security Advisories: https://github.com/flarecentury/mlpcopilot/security/advisories
- Release Notes: https://github.com/flarecentury/mlpcopilot/releases
