...
export function ChatPane({ session, onNewChat }: ChatPaneProps) {
  ...
  const { messages, isStreaming, send, setMessages } = useMLPCopilotStream(
    chatId,
    initial,
  );
      ...
      <section className="flex min-h-0 flex-1 flex-col">
        <div className="flex flex-1 flex-col items-center justify-center gap-8 px-4 pb-6">
          <div className="flex flex-col items-center gap-4 animate-in fade-in-0 slide-in-from-bottom-2 duration-500">
            <picture>
              <source
                srcSet="/brand/mlpcopilot_logo.webp"
                type="image/webp"
              />
              <img
                src="/brand/mlpcopilot_logo.png"
                alt="mlpcopilot"
            ...
            <p className="max-w-md text-center text-sm text-muted-foreground">
              Your conversations are persisted locally under the mlpcopilot
