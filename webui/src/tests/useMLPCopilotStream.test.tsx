...
import { useMLPCopilotStream } from "@/hooks/useMLPCopilotStream";
...
function wrap(client: ReturnType<typeof fakeClient>["client"]) {
  return function Wrapper({ children }: { children: ReactNode }) {
    return (
      <ClientProvider
        client={client as unknown as import("@/lib/mlpcopilot-client").MLPCopilotClient}
        token="tok"
      >
        {children}
      </ClientProvider>
...
describe("useMLPCopilotStream", () => {
  it("collapses consecutive tool_hint frames into one trace row", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useMLPCopilotStream("chat-t", []), {
  ...
  it("attaches assistant media_urls to complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useMLPCopilotStream("chat-m", []), {
      wrapper: wrap(fake.client),
    });
  ...
  it("keeps assistant buttons on complete messages", () => {
    const fake = fakeClient();
    const { result } = renderHook(() => useMLPCopilotStream("chat-q", []), {
      wrapper: wrap(fake.client),
    });
