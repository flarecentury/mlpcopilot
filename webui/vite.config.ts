...
export default defineConfig(({ mode }) => {
  ...
  const target = env.MLPCOPILOT_API_URL ?? "http://127.0.0.1:8765";
  ...
  return {
    ...
    build: {
      outDir: path.resolve(__dirname, "../mlpcopilot/web/dist"),
      emptyOutDir: true,
      sourcemap: false,
    },
    server: {
      ...
      // the ``/`` proxy below (Vite HMR and the mlpcopilot ws upgrade both sit on
      ...
      proxy: {
        ...
        // Forward only WebSocket upgrades on ``/`` to the mlpcopilot gateway;
