...
The browser front-end for the mlpcopilot gateway. It is built with Vite + React 18 +
...
mlpcopilot/web/dist/      build output served by the gateway
```

## Develop from source

### 1. Install mlpcopilot from source
...
```bash
MLPCOPILOT_API_URL=http://127.0.0.1:9000 bun run dev
```
...
This writes the production assets to `../mlpcopilot/web/dist`, which is the
directory served by `mlpcopilot gateway` and bundled into the Python wheel.
