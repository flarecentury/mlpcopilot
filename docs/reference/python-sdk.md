...
Use the mlpcopilot distribution as a library — no CLI, no gateway, just Python.
...
from mlpcopilot import MLPCopilot


async def main() -> None:
    bot = MLPCopilot.from_config()
...
`MLPCopilot.from_config()` reuses your normal `~/.mlpcopilot/config.json`, so the SDK follows the same provider, model, tools, and workspace defaults as the CLI unless you override them.
...
bot = MLPCopilot.from_config(
    config_path="~/.mlpcopilot/config.json",
...
Hooks let you inspect tool calls, streaming, and iteration state without modifying mlpcopilot internals:

```python
from mlpcopilot.agent import AgentHook, AgentHookContext
...
### `MLPCopilot.from_config(config_path=None, *, workspace=None)`

Create a `MLPCopilot` instance from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.mlpcopilot/config.json`. |
...
```python
from mlpcopilot.agent import AgentHook
...
async def main() -> None:
    bot = MLPCopilot.from_config(workspace="/my/project")
