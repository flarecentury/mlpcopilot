...
Config file: `~/.mlpcopilot/config.json`
...
The `mlpcopilot` profile is the host runtime profile for MLP Copilot workspaces. It defaults to workspace-restricted tools, sends agent-side built-in and MCP tool calls through ApprovalManager unless the exact tool name is allowlisted, configures `exec` to use its own exact command allowlist and approval flow if explicitly enabled, disables web/exec/spawn/notebook/cron-style built-ins, keeps Telegram as the only remote gateway channel, discovers source-tree MLP MCP servers when `tools.mcpServers` is absent, and initializes the MLP Copilot workspace schema. Use:

```bash
mlpcopilot onboard --workspace ~/.mlpcopilot/workspace
```

Use separate config files when you want multiple MLP Copilot instances with different workspaces or policies:

```bash
mlpcopilot mlp init -c ~/.mlpcopilot/instances/lab/config.json -w ~/.mlpcopilot/instances/lab/workspace
mlpcopilot tui -c ~/.mlpcopilot/instances/lab/config.json
mlpcopilot tui -c ~/.mlpcopilot/config.json
```

The TUI Companion and Artifacts panes read generic adapter display documents from the active project/run UI directory, such as `projects/<project_id>/runs/<run_id>/ui/companion.display.json` and `artifacts.display.json`. If no Companion display document exists, the TUI falls back to workspace-local campaign status files:

```json
{
  "tui": {
    "companionStaleAfterSeconds": 3600,
    "campaignStatusPaths": [
...
`companionStaleAfterSeconds` controls when the Companion pane marks display-document timestamps as stale. Set it to `0` to disable stale marking.

Scientific workflow adapters should prefer display documents when they need full control over the pane layout. The campaign status fallback is only a generic runtime read model for status references.

TUI shortcut keys are configurable under `tui.keymap`. This is useful in browser-backed terminals where keys like `Ctrl-T` may be intercepted by Firefox or Jupyter. Values can be one key, multiple alternative keys, or an empty list to disable that action:
...
Key names follow prompt-toolkit conventions. Common aliases such as `ctrl-t`, `c-t`, `pgup`, and `page-up` are accepted. Multi-key chords can be written with spaces, for example `"ctrl-x ctrl-t"`.

To enable `exec` for this profile, opt in explicitly and provide command regexes:

```json
{
  "tools": {
    "enabledBuiltinTools": ["ask_user", "my", "read_file", "file_info", "list_dir", "grep", "glob", "web_search", "web_fetch", "write_file", "edit_file", "message", "workstate", "exec"],
    "exec": {
      "enable": true,
      "requireAllowlist": true,
      "approvalRequired": true,
      "allowPatterns": ["^pytest\\b", "^python -m pytest\\b"]
    }
  }
}
```
...
```ini
# /etc/systemd/system/mlpcopilot.service (excerpt)
[Service]
EnvironmentFile=/home/youruser/mlpcopilot_secrets.env
User=mlpcopilot
ExecStart=...
```

```bash
# /home/youruser/mlpcopilot_secrets.env (mode 600, owned by youruser)
...
> - **MiniMax Coding Plan**: Exclusive discount links for the mlpcopilot community: [Overseas](https://platform.minimax.io/subscribe/coding-plan?code=9txpdXw04g&source=link) · [Mainland China](https://platform.minimaxi.com/subscribe/token-plan?code=GILTJpMTqZ&source=link)
> - **MiniMax (Mainland China)**: If your API key is from MiniMax's mainland China platform (minimaxi.com), set `"apiBase": "https://api.minimaxi.com/v1"` in your minimax provider config.
> - **MiniMax thinking mode**: Use `providers.minimaxAnthropic` when you want `reasoningEffort` / thinking mode. MiniMax exposes that capability through its Anthropic-compatible endpoint, so mlpcopilot keeps it as a separate provider instead of guessing MiniMax-specific thinking parameters on the generic OpenAI-compatible `minimax` endpoint. It uses the same `MINIMAX_API_KEY`. Default Anthropic-compatible base URL: `https://api.minimax.io/anthropic`; for mainland China use `https://api.minimaxi.com/anthropic`.
...
| `volcengine` | LLM (VolcEngine, pay-per-use) | [Coding Plan](https://www.volcengine.com/activity/codingplan?utm_campaign=mlpcopilot&utm_content=mlpcopilot&utm_medium=devrel&utm_source=OWO&utm_term=mlpcopilot) · [volcengine.com](https://www.volcengine.com) |
| `byteplus` | LLM (VolcEngine international, pay-per-use) | [Coding Plan](https://www.byteplus.com/en/activity/codingplan?utm_campaign=mlpcopilot&utm_content=mlpcopilot&utm_medium=devrel&utm_source=OWO&utm_term=mlpcopilot) · [byteplus.com](https://www.byteplus.com) |
...
| `stepfun` | LLM (StepFun) | [platform.stepfun.com](https://platform.stepfun.com) |
| `ovms` | LLM (local, OpenVINO Model Server) | [docs.openvino.ai](https://docs.openvino.ai/2026/model-server/ovms_docs_llm_quickstart.html) |
| `vllm` | LLM (local, any OpenAI-compatible server) | — |
| `openai_codex` | LLM (Codex, OAuth) | `mlpcopilot provider login openai-codex` |
| `github_copilot` | LLM (GitHub Copilot, OAuth) | `mlpcopilot provider login github-copilot` |
...
You can also set `providers.bedrock.apiKey` to a Bedrock API key; mlpcopilot exports it as `AWS_BEARER_TOKEN_BEDROCK` for the AWS SDK.
...
Claude Opus 4.7 does not accept `temperature`, `top_p`, or `top_k`; mlpcopilot omits `temperature` automatically for this model. If `reasoningEffort` is set to `low`, `medium`, `high`, `max`, or `adaptive`, mlpcopilot sends Bedrock's adaptive thinking parameter.
...
Use Bedrock model IDs or inference profile IDs with a `bedrock/` prefix in mlpcopilot config. mlpcopilot removes the prefix before calling AWS.
...
Model-specific fields can be supplied with `extraBody`; mlpcopilot merges it into Converse `additionalModelRequestFields`:
...
Current scope: mlpcopilot passes `messages`, `system`, `inferenceConfig`, `toolConfig`, and `additionalModelRequestFields`. Bedrock Prompt Management, Guardrails, `serviceTier`, and other top-level Converse options are not first-class config fields yet.
...
```bash
mlpcopilot agent -m "Reply with one short sentence."
```
...
No `providers.openaiCodex` block is needed in `config.json`; `mlpcopilot provider login` stores the OAuth session outside config.

**1. Login:**
```bash
mlpcopilot provider login openai-codex
```

**2. Set model** (merge into `~/.mlpcopilot/config.json`):
```json
{
  "agents": {
    "defaults": {
...
**3. Chat:**
```bash
mlpcopilot agent -m "Hello!"

# Target a specific workspace/config locally
mlpcopilot agent -c ~/.mlpcopilot-telegram/config.json -m "Hello!"

# One-off workspace override on top of that config
mlpcopilot agent -c ~/.mlpcopilot-telegram/config.json -w /tmp/mlpcopilot-telegram-test -m "Hello!"
```
...
No `providers.githubCopilot` block is needed in `config.json`; `mlpcopilot provider login` stores the OAuth session outside config.

**1. Login:**
```bash
mlpcopilot provider login github-copilot
```
...
LongCat is available through mlpcopilot's built-in OpenAI-compatible provider flow.
...
Some OpenAI-compatible gateways expose request-body extensions such as vLLM guided decoding or local sampling controls. Put those under `extraBody`; mlpcopilot merges them into the chat-completions request body after its provider defaults:
...
**2. Add to config** (partial — merge into `~/.mlpcopilot/config.json`):
```json
{
  "providers": {
    "ollama": {
...
**3. Add to config** (partial — merge into `~/.mlpcopilot/config.json`):
...
mlpcopilot uses a **Provider Registry** (`mlpcopilot/providers/registry.py`) as the single source of truth.
Adding a new provider only takes **2 steps** — no if-elif chains to touch.

**Step 1.** Add a `ProviderSpec` entry to `PROVIDERS` in `mlpcopilot/providers/registry.py`:
...
ProviderSpec(
    ...
    display_name="My Provider",          # shown in `mlpcopilot status`
    default_api_base="https://api.myprovider.com/v1",  # OpenAI-compatible endpoint
)
```

**Step 2.** Add a field to `ProvidersConfig` in `mlpcopilot/config/schema.py`:
...
That's it! Environment variables, model routing, config matching, and `mlpcopilot status` display will all work automatically.
...
Global settings that apply to all channels. Configure under the `channels` section in `~/.mlpcopilot/config.json`:
...
When a channel `send()` raises, mlpcopilot retries at the channel-manager layer. By default, `channels.sendMaxRetries` is `3`, and that count includes the initial send.
...
>
> If a channel is completely unreachable, mlpcopilot cannot notify the user through that same channel. Watch logs for `Failed to send to {channel} after N attempts` to spot persistent delivery failures.

## Web Tools

mlpcopilot incorporates basic tools for accessing the web. These include searching via APIs, and fetching arbitrary web pages in Markdown format. They are enabled by default, and can be configured in `~/.mlpcopilot/config.json` under `tools.web`.
...
mlpcopilot supports multiple web search providers. Configure in `~/.mlpcopilot/config.json` under `tools.web.search`.
...
mlpcopilot by default uses [Jina Reader](https://jina.ai/reader/), a third-party API, to convert arbitrary pages into Markdown format for easy digestion by the LLM, with a local fallback based on [readability-lxml](https://github.com/buriy/python-readability) if the former fails.
...
mlpcopilot supports [MCP](https://modelcontextprotocol.io/) — connect external tool servers and use them as native agent tools.
...
MCP tool calls default to a 120s per-call timeout. Use `toolTimeout` to override
that value for a specific slow or fast-fail server:
...
`enabledTools` accepts either the raw MCP tool name (for example `read_file`) or the wrapped mlpcopilot tool name (for example `mcp_filesystem_write_file`).
...
|--------|---------|-------------|
| `tools.restrictToWorkspace` | `false` | When `true`, restricts file tools to workspace paths. Workspace symlink targets may resolve outside the workspace for read-only inspection, but paths must be entered through the workspace. |
| `tools.readAllowlist` | `[]` | Extra read-only roots or files that local read/search tools may inspect without ApprovalManager approval when workspace restriction is enabled. Entries may be absolute paths or workspace-relative paths. This does not allow writes. |
| `tools.approvalRequiredForTools` | `false` | Require ApprovalManager approval before agent-side tool calls. Exact `tools.approvalAllowlist` entries bypass this gate; the `mlpcopilot` profile allowlists basic read/status tools by default, including local read/search tools and `web_search`/`web_fetch` when web tools are explicitly enabled. MCP tools are also allowed without approval when the server exposes the standard MCP `ToolAnnotations.readOnlyHint=true` and does not mark the tool destructive; mutating or unannotated MCP tools stay gated unless explicitly allowlisted. |
...
| `tools.exec.backgroundCommands` | `["cmatrix", "top", "htop", "btop", "watch"]` | Command names that run in background by default when invoked without shell chaining or redirection. The `exec` tool also accepts `background: true` for long-running jobs whose live output is not needed in chat; stdout/stderr are written under `workspace/jobs/`. |
| `channels.*.allowFrom` | `[]` (deny all) | Whitelist of user IDs. Empty denies all; use `["*"]` to allow everyone. |

**Docker security**: The official Docker image runs as a non-root user (`mlpcopilot`, UID 1000) with bubblewrap pre-installed. When using `docker-compose.yml`, the container drops all Linux capabilities except `SYS_ADMIN` (required for bwrap's namespace isolation).


## Auto Compact

When a user is idle for longer than a configured threshold, mlpcopilot **proactively** compresses the older part of the session context into a summary while keeping a recent legal suffix of live messages. This reduces token cost and first-token latency when the user returns — instead of re-processing a long stale context with an expired KV cache, the model receives a compact summary, the most recent live context, and fresh input.
...
By default, mlpcopilot uses `UTC` for runtime time context. If you want the agent to think in your local time, set `agents.defaults.timezone` to a valid [IANA timezone name](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones):
...
By default, each channel × chat ID combination gets its own session. If you use mlpcopilot across multiple channels (e.g. Telegram + Discord + CLI) and want them to share the same conversation, enable `unifiedSession`:
...
mlpcopilot ships with built-in skills, and your workspace can also define custom skills under `skills/`. If you want to hide specific skills from the agent, set `agents.defaults.disabledSkills` to a list of skill directory names:
