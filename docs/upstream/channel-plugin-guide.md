...
Build a custom mlpcopilot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of mlpcopilot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

mlpcopilot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `mlpcopilot gateway` starts, it scans:

1. Built-in channels in `mlpcopilot/channels/`
2. External packages registered under the `mlpcopilot.channels` entry point group
...
```text
mlpcopilot-channel-webhook/
├── mlpcopilot_channel_webhook/
...
```python
# mlpcopilot_channel_webhook/__init__.py
from mlpcopilot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# mlpcopilot_channel_webhook/channel.py
import asyncio
...
from mlpcopilot.config.schema import Base
...
# pyproject.toml
[project]
name = "mlpcopilot-channel-webhook"
version = "0.1.0"
dependencies = ["mlpcopilot", "aiohttp"]

[project.entry-points."mlpcopilot.channels"]
webhook = "mlpcopilot_channel_webhook:WebhookChannel"
...
packages = ["mlpcopilot_channel_webhook"]
```
...
```bash
pip install -e .
mlpcopilot plugins list      # verify "Webhook" shows as "plugin"
mlpcopilot onboard           # auto-adds default config for detected plugins
```

Edit `~/.mlpcopilot/config.json`:
...
```bash
mlpcopilot channels login <channel_name>
mlpcopilot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `mlpcopilot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `mlpcopilot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `mlpcopilot.config.schema.Base`:
...
Override `default_config()` so `mlpcopilot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `mlpcopilot-channel-{name}` | `mlpcopilot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `mlpcopilot_channel_{name}` | `mlpcopilot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/mlpcopilot-channel-webhook
cd mlpcopilot-channel-webhook
pip install -e .
mlpcopilot plugins list    # should show "Webhook" as "plugin"
mlpcopilot gateway         # test end-to-end
```

## Verify

```bash
$ mlpcopilot plugins list
