...
Connect mlpcopilot to your favorite chat platform. Want to build your own? See the [Channel Plugin Guide](./channel-plugin-guide.md).

| Channel | What you need |
|---------|---------------|
| **Telegram** | Bot token from @BotFather |
| **Discord** | Bot token + Message Content intent |
| **WhatsApp** | QR code scan (`mlpcopilot channels login whatsapp`) |
| **WeChat (Weixin)** | QR code scan (`mlpcopilot channels login weixin`) |
...
**1. Ask mlpcopilot to set up Mochat for you**

Simply send this message to mlpcopilot (replace `xxx@xxx` with your real email):

```
Read the MoChat mlpcopilot skill instructions and register on MoChat. My email account is xxx@xxx. Bind me as your owner and DM me on MoChat.
```

mlpcopilot will automatically register, configure `~/.mlpcopilot/config.json`, and connect to Mochat.
...
That's it — mlpcopilot handles the rest!
...
If you prefer to configure manually, add the following to `~/.mlpcopilot/config.json`:
...
- You need:
  - `userId` (example: `@mlpcopilot:matrix.org`)
  - `password`
...
```bash
mlpcopilot channels login whatsapp
...
> After upgrading mlpcopilot, rebuild the local bridge with:
> `rm -rf ~/.mlpcopilot/bridge && mlpcopilot channels login whatsapp`
...
- **Permissions**:
  ...
  - **Streaming replies** (default in mlpcopilot): add **`cardkit:card:write`** (often labeled **Create and update cards** in the Feishu developer console). Required for CardKit entities and streamed assistant text. Older apps may not have it yet — open **Permission management**, enable the scope, then **publish** a new app version if the console requires it.
  - If you **cannot** add `cardkit:card:write`, set `"streaming": false` under `channels.feishu` (see below). The bot still works; replies use normal interactive cards without token-by-token streaming.
- **Events**: Add `im.message.receive_v1` (receive messages)
  - Select **Long Connection** mode (requires running mlpcopilot first to establish connection)
...
> `allowFrom`: Add your open_id (find it in mlpcopilot logs when you message the bot). Use `["*"]` to allow all users.
...
> - `allowFrom`: Add your openid (find it in mlpcopilot logs when you message the bot). Use `["*"]` for public access.
...
> `files:read` is required to read files users send to mlpcopilot. `files:write` is required for mlpcopilot to send images, videos, and other file uploads. If you add either scope later, reinstall the Slack app to the workspace and restart mlpcopilot so it uses the updated bot token.

**3. Configure mlpcopilot**
...
Give mlpcopilot its own email account. It polls **IMAP** for incoming mail and replies via **SMTP** — like a personal email assistant.

**1. Get credentials (Gmail example)**
- Create a dedicated Gmail account for your bot (e.g. `my-mlpcopilot@gmail.com`)
...
```json
{
  "channels": {
    "email": {
      ...
      "imapPort": 993,
      "imapUsername": "my-mlpcopilot@gmail.com",
      "imapPassword": "your-app-password",
      "smtpHost": "smtp.gmail.com",
      "smtpPort": 587,
      "smtpUsername": "my-mlpcopilot@gmail.com",
      "smtpPassword": "your-app-password",
      "fromAddress": "my-mlpcopilot@gmail.com",
...
> - `allowFrom`: Add the sender ID you see in mlpcopilot logs for your WeChat account. Use `["*"]` to allow all users.
> - `token`: Optional. If omitted, log in interactively and mlpcopilot will save the token for you.
> - `routeTag`: Optional. When your upstream Weixin deployment requires request routing, mlpcopilot will send it as the `SKRouteTag` header.
> - `stateDir`: Optional. Defaults to mlpcopilot's runtime directory for Weixin state.
> - `pollTimeout`: Optional long-poll timeout in seconds.

**3. Login**

```bash
mlpcopilot channels login weixin
```

Use `--force` to re-authenticate and ignore any saved token:

```bash
mlpcopilot channels login weixin --force
```
...
```bash
pip install mlpcopilot[msteams]
```
...
> - `mentionOnlyResponse` controls what MLP Copilot receives when a user sends only a bot mention (`<at>MLP Copilot</at>`). Set to `""` to ignore mention-only messages.
